  ...
  val tests = TestSuite {
    ...
    'parse {
      ...
      'basic {
        ...
        assert(
          itv1 == Some(VersionInterval(Some(Version("1.1")), Some(Version("1.3")), true, true))
        )

        val s2 = "(1.1,1.3]"
        val itv2 = Parse.versionInterval(s2)
        assert(
          itv2 == Some(VersionInterval(Some(Version("1.1")), Some(Version("1.3")), false, true))
        )

        val s3 = "[1.1,1.3)"
        val itv3 = Parse.versionInterval(s3)
        assert(
          itv3 == Some(VersionInterval(Some(Version("1.1")), Some(Version("1.3")), true, false))
        )

        val s4 = "(1.1,1.3)"
        val itv4 = Parse.versionInterval(s4)
        assert(
          itv4 == Some(VersionInterval(Some(Version("1.1")), Some(Version("1.3")), false, false))
        )
        ...
        assert(
          itv5 == Some(
            VersionInterval(Some(Version("1.11.0")), Some(Version("1.12.0")), false, true)
          )
        )
      }
      'leftEmptyVersions {
      ...
      'fixedVersion - {
        * - {
          ...
          assert(
            itv == Some(VersionInterval(Some(Version("1.2")), Some(Version("1.2")), true, true))
          )
        }
    ...
    'constraint {
      ...
      'interval {
        ...
        assert(
          c1 == Some(
            VersionConstraint
              .interval(VersionInterval(Parse.version("1.3"), Parse.version("2.4"), true, false))
          )
        )
      }
    }
