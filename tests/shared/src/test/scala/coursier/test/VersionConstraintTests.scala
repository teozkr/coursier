  ...
  val tests = TestSuite {
    'parse {
      ...
      'basicVersionInterval {
        ...
        assert(
          c0 == Some(
            VersionConstraint.interval(VersionInterval(None, Some(Version("1.2")), false, true))
          )
        )
      }
      'latestSubRevision {
        ...
        assert(
          c0 == Some(
            VersionConstraint.interval(
              VersionInterval(Some(Version("1.2.3")), Some(Version("1.2.4-a1")), true, false)
            )
          )
        )
      }
    ...
    'repr {
      ...
      'interval {
        val s0 =
          VersionConstraint.interval(VersionInterval(None, Some(Version("2.1")), false, true)).repr
        assert(s0 == Some("(,2.1]"))
      }
    }
  }
