...
object ResolutionTests extends TestSuite {
  ...
  def resolve0(
    ...
    Resolution(deps, filter = filter, forceVersions = forceVersions).process
      .run(Platform.fetch(repositories))
      .runF

  implicit class ProjectOps(val p: Project) extends AnyVal {
    def kv: (ModuleVersion, (Artifact.Source, Project)) =
      p.moduleVersion -> (testRepository.source, p)
  }
  ...
  val projects = Seq(
    ...
    Project(
      Module("acme", "play"),
      "2.4.0",
      Seq("" -> Dependency(Module("acme", "play-json"), "2.4.0"))
    ),
    ...
    Project(
      ...
      properties = Seq("play_json_version" -> "2.4.0", "WithSpecialChar©" -> "config")
    ),
    Project(
      Module("acme", "play-extra-no-config"),
      "2.4.1",
      Seq("" -> Dependency(Module("acme", "play"), "2.4.1", exclusions = Set(("acme", "config"))))
    ),
    Project(
      Module("acme", "play-extra-no-config-no"),
      "2.4.1",
      Seq("" -> Dependency(Module("acme", "play"), "2.4.1", exclusions = Set(("*", "config"))))
    ),
    Project(
      Module("acme", "module-with-missing-pom"),
      "1.0.0",
      dependencyManagement = Seq("import" -> Dependency(Module("acme", "missing-pom"), "1.0.0"))
    ),
    Project(
      Module("hudsucker", "mail"),
      "10.0",
      Seq("test" -> Dependency(Module("${project.groupId}", "test-util"), "${project.version}"))
    ),
    Project(Module("hudsucker", "test-util"), "10.0"),
    Project(
      Module("se.ikea", "parent"),
      "18.0",
      dependencyManagement = Seq(
        "" -> Dependency(Module("acme", "play"), "2.4.0", exclusions = Set(("acme", "play-json")))
      )
    ),
    Project(
      Module("se.ikea", "billy"),
      "18.0",
      dependencies = Seq("" -> Dependency(Module("acme", "play"), "")),
      parent = Some(Module("se.ikea", "parent"), "18.0")
    ),
    Project(
      Module("org.gnome", "parent"),
      "7.0",
      Seq("" -> Dependency(Module("org.gnu", "glib"), "13.4"))
    ),
    Project(
      Module("org.gnome", "panel-legacy"),
      "7.0",
      dependencies = Seq("" -> Dependency(Module("org.gnome", "desktop"), "${project.version}")),
      parent = Some(Module("org.gnome", "parent"), "7.0")
    ),
    Project(
      Module("gov.nsa", "secure-pgp"),
      "10.0",
      Seq("" -> Dependency(Module("gov.nsa", "crypto"), "536.89"))
    ),
    Project(
      Module("com.mailapp", "mail-client"),
      "2.1",
      dependencies = Seq(
        "" -> Dependency(
          Module("gov.nsa", "secure-pgp"),
    ...
    Project(
      Module("com.thoughtworks.paranamer", "paranamer-parent"),
      "2.6",
      dependencies = Seq("" -> Dependency(Module("junit", "junit"), "")),
      dependencyManagement = Seq("test" -> Dependency(Module("junit", "junit"), "4.11"))
    ),
    Project(
    ...
    Project(
      ...
      profiles = Seq(
        Profile(
          ...
          dependencies =
            Seq("" -> Dependency(Module("org.escalier", "librairie-standard"), "2.11.6"))
        )
      )
    ),
    Project(
      ...
      "0.4.2",
      dependencies =
        Seq("" -> Dependency(Module("org.scalaverification", "scala-verification"), "1.12.4")),
      profiles = Seq(
        Profile(
          "default",
          activeByDefault = Some(true),
    ...
    Project(
      Module("com.github.dummy", "libb-parent"),
      "0.5.6",
      properties = Seq("special" -> "true")
    ),
    Project(
    ...
    Project(
      Module("an-org", "an-app"),
      "1.1",
      Seq("" -> Dependency(Module("an-org", "a-lib"), "1.1"))
    ),
    Project(
      Module("an-org", "an-app"),
      "1.2",
      Seq("" -> Dependency(Module("an-org", "a-lib"), "1.2"))
    )
  )

  val projectsMap = projects
    .map(p => p.moduleVersion -> p.copy(configurations = MavenRepository.defaultConfigurations))
    .toMap
  ...
  val tests = TestSuite {
    ...
    'missingPom {
      async {
        ...
        assert(
          res.metadataErrors == Seq((Module("acme", "missing-pom"), "1.0.0") -> List("Not found"))
        )
      }
    }
    'single {
      async {
        ...
        val expected = Resolution(
          ...
          dependencies = Set(dep.withCompileScope),
          projectCache =
            Map(dep.moduleVersion -> (testRepository.source, projectsMap(dep.moduleVersion)))
        )
    ...
    'exclude {
      async {
        ...
        val trDeps = Seq(
          Dependency(Module("acme", "play"), "2.4.1", exclusions = Set(("acme", "config"))),
          Dependency(Module("acme", "play-json"), "2.4.0", exclusions = Set(("acme", "config")))
        )
        val res = await(
          resolve0(
            Set(dep)
    ...
    'excludeOrgWildcard {
      async {
        ...
        val trDeps = Seq(
          Dependency(Module("acme", "play"), "2.4.1", exclusions = Set(("*", "config"))),
    ...
    'depsFromDefaultProfile {
      async {
        ...
        val trDeps = Seq(Dependency(Module("org.escalier", "librairie-standard"), "2.11.6"))
        val res = await(
          resolve0(
            Set(dep)
          )
    ...
    'dependencyOverrides - {
      * - {
        async {
          val deps = Set(Dependency(Module("an-org", "a-name"), "1.1"))
          val depOverrides = Map(Module("an-org", "a-name") -> "1.0")
      ...
      * - {
        async {
          val deps = Set(Dependency(Module("an-org", "an-app"), "1.1"))
          val depOverrides = Map(Module("an-org", "a-lib") -> "1.0")
      ...
      * - {
        async {
          val deps = Set(Dependency(Module("an-org", "an-app"), "1.2"))
          val depOverrides = Map(Module("an-org", "a-lib") -> "1.1")
