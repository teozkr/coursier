...
case class Scaladex[F[_]](fetch: String => EitherT[F, String, String], F: Nondeterminism[F]) {
  ...
  def search(
    ...
    target: String,
    scalaVersion: String
  ): EitherT[F, String, Seq[Scaladex.SearchResult]] = {
    ...
    s.flatMap(
      s => EitherT.fromDisjunction[F](s.decodeEither[List[Scaladex.SearchResult]].disjunction)
    )
  }
  ...
  def artifactInfos(
    ...
    repository: String,
    artifactName: String
  ): EitherT[F, String, Scaladex.ArtifactInfos] = {
  ...
  def dependencies(
    ...
    val orgNameOrError =
      if (idx >= 0) {
        ...
        artifactNames(org, repo).map((org, repo, _)): EitherT[
          F,
          String,
        ...
        search(name, "JVM", scalaVersion) // FIXME Don't hardcode
          .flatMap {
            case Seq(first, _*) =>
              ...
              ): EitherT[F, String, (String, String, Seq[String])]
            case Seq() =>
              EitherT.fromDisjunction[F](s"No project found for $name".left): EitherT[
                F,
                String,
                (String, String, Seq[String])
              ]
      ...
      case (ghOrg, ghRepo, artifactNames) =>
        val moduleVersions = F.map(F.gather(artifactNames.map { artifactName =>
          F.map(artifactInfos(ghOrg, ghRepo, artifactName).run) {
            case -\/(err) =>
              logger(
                s"Cannot get infos about artifact $artifactName from $ghOrg/$ghRepo: $err, ignoring it"
              )
              Nil
            case \/-(infos) =>
